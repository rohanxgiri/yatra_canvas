...
- Progressive POI Prefetch & Cache-First Live Discovery Reliability (`CityPlacePrefetchService`, `OpenStreetMapDiscoveryService`, `GeoapifyPlacesProvider`, `ProviderCircuitBreaker`):
  - **3-Tier Cache Semantics**: Queries evaluate category coverage into `FRESH` ($\le 24$h / `CACHE_TTL_HOURS`), `STALE_USABLE` ($\le 168$h / `DISCOVERY_STALE_USABLE_HOURS`), and `MISSING`. If category coverage meets `DISCOVERY_MIN_USABLE_CANDIDATES_PER_CATEGORY` (default 6) via fresh or stale-usable cached places, recommendations are served immediately without blocking on external network calls.
  - **Asynchronous Revalidation**: Stale-usable categories immediately return cached places to the user while background revalidation is scheduled via `asyncio.create_task()` or FastAPI background tasks.
  - **In-Memory Concurrency Deduplication**: `CityPlacePrefetchService` maintains an in-flight future registry keyed by `(city_id, category)`. Concurrent user requests or overlapping prefetch triggers for the same city/category await the in-flight future or reuse available cache rather than duplicating external network requests.
  - **Provider Hierarchy & Circuit Breaker**: Discovery queries follow a resilient fallback sequence: Stored DB Cache $\to$ `GeoapifyPlacesProvider` (hosted Geoapify `/v2/places` API with bounded coordinate radius) $\to$ `AudialaPlacesProvider` (local enriched POI dataset) $\to$ `OpenStreetMapPlacesService` (Overpass OSM). Overpass is protected by an in-memory `ProviderCircuitBreaker` tracking consecutive failures and enforcing a cooldown window (`OVERPASS_CIRCUIT_BREAKER_THRESHOLD=3`, `OVERPASS_CIRCUIT_BREAKER_COOLDOWN_SECONDS=60.0s`). If Overpass is open or times out, discovery gracefully succeeds using Geoapify and Audiala.
  - **Progressive Prefetch Lifecycle**:
    1. *Destination Confirmed (`shallow`)*: User taps Continue on destination selection $\to$ fires non-blocking `POST /places/prefetch` with stage `shallow` fetching core categories (`tourism`, `heritage`, `food`, `religious`, `cafes`) with a bounded limit (`DISCOVERY_SHALLOW_LIMIT=15`). Navigation to dates proceeds instantaneously without awaiting network.
    2. *Purposes/Interests Confirmed (`targeted`)*: User confirms trip purposes $\to$ fires non-blocking `POST /places/prefetch` with stage `targeted` containing only categories mapped from user interests. Inspects current category coverage and enriches only missing or low-coverage categories.
