...
| Variable | Provider | Required/Optional/Unused | Location | Backend/Client | Secret? | Used by | Missing behaviour | Recommended action |
| -------- | -------- | ------------------------ | -------- | -------------- | ------- | ------- | ----------------- | ------------------ |
| `GOOGLE_PLACES_API_KEY` | Google Places API (New) | Optional implemented integration | `backend/app/core/config.py`, `backend/app/routers/cities.py`, `backend/app/services/google_places_service.py`, `backend/.env.example` | Backend only | Yes | City autocomplete/details, nearby POI discovery, recommendations that refresh discovery | Provider-backed calls return a safe `503`; stored city/place operations still work | Add only if these features are needed; restrict the key to Places API (New) and the backend deployment |
| `GOOGLE_ROUTES_API_KEY` | Google Routes API | Optional implemented integration | `backend/app/core/config.py`, `backend/app/routers/route_optimization.py`, `backend/app/services/google_routes_service.py`, `backend/.env.example` | Backend only | Yes | Route-matrix fetches for itinerary optimization | New or incomplete matrices return a safe `503`; a complete cached matrix can still be reused | Add only for route optimization; use a separately restricted key where practical |
...
| No variable declared | OpenStreetMap/Overpass live client | Planned, not implemented | Intended architecture only | Likely backend | Normally no runtime key | Nothing in current code | No live OSM/Overpass enrichment | Implement rate-limit/caching policy before adding configuration |
...
| Variable | Provider | Required/Optional/Unused | Location | Backend/Client | Secret? | Used by | Missing behaviour | Recommended action |
| -------- | -------- | ------------------------ | -------- | -------------- | ------- | ------- | ----------------- | ------------------ |
| `GOOGLE_MAPS_API_KEY` | Google Maps SDK | Verified unused / not declared | No Maps SDK dependency, manifest metadata, or source configuration | Neither | Yes if one existed | Nothing; UI text saying “Google Maps” accompanies Google Places results but does not initialize a map SDK | None | Do not add it; manually remove it from local environments if present and not used by external infrastructure |
...
| No variable | OpenStreetMap-derived data/attribution | Used indirectly; no live API client | Place provenance and Flutter attribution | Backend data / client attribution | No | Attribution and OSM-derived provider context | No live OSM/Overpass fetch exists | Preserve attribution; add no key |
| No variable | Overpass | Planned only | Intended architecture | Backend when implemented | No | Nothing currently | No additional POI fetch | Use a keyless endpoint with responsible caching/rate limits when implemented |
