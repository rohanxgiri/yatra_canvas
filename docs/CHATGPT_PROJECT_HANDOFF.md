...
- `[IMPLEMENTED]` **Recommendation Pipeline:** Canonical and spatial deduplication ($\le 75$m), presentation-level brand capping, Levenshtein typo tolerance, traveller suitability / access confidence filtering (excluding institutional canteens, private offices, and student messes via operator/building context), bounded Wikidata importance, purpose/interest scoring, and mixed-interest category balancing hardened across 7 benchmark cities with 0 restricted POI leakage.
...
- `[IMPLEMENTED]` **Destination-Scoped Manual Place Search:** 350ms debounced search (`GET /cities/{city_id}/places/search`) across destination-radius Geoapify and local DB, canonical Place resolution (`POST /cities/{city_id}/places/resolve`) via `CanonicalPlaceService`, duplicate prevention, and route eligibility.
- `[IMPLEMENTED]` **Multi-Day Itinerary Optimization & Logical Day Invariant:** Google OR-Tools VRPTW solver returning `total_days=trip.days`, daily touring budgets (`09:00–19:00`), category visit durations, lunch breaks (`12:30–14:00`), opening hours, and locked stops. Complete logical day sequence ($1 \dots N$) is preserved across models, itinerary empty day cards, and map day selectors even when intermediate days have 0 stops.
...
- `[IMPLEMENTED]` **Progressive Interactive Map:** `FlutterMap` widget with OpenStreetMap tiles, start/place markers, day-filtered road polylines, and Frame 1 progressive rendering ($12-25\text{ ms}$) via pre-passed trip state and non-blocking asynchronous route geometry loading.
