...
| Discover/recommend places | `[IMPLEMENTED]` | The recommendation flow uses a deterministic multi-stage pipeline: canonical & spatial deduplication (resolving node/way/multi-category duplicates while preserving separate chain branches), general context-based traveller suitability and access confidence filtering (excluding internal canteens, student messes, and staff facilities without hardcoding institution names), food intent prioritization, and diversity ranking with explainable recommendation reasons. |
