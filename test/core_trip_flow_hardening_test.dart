...
class _FakePlaceService implements PlaceService {
  ...
  Future<List<PlaceSearchResult>> searchPlaces(
    ...
    if (failSearch) {
      throw const PlaceServiceException('Location search is unavailable.');
    }
  ...
  Future<TripRouteGeometry> getRouteGeometry(
    ...
    if (fail) {
      throw const RouteGeometryServiceException(
        'Route geometry provider offline',
      );
    }
    return const TripRouteGeometry(
      ...
      days: [
        DayRouteGeometry(
          dayNumber: 1,
          points: [LatLng(9.95, 76.25), LatLng(9.96, 76.24)],
          distanceMeters: 1000,
          durationSeconds: 600,
        ),
      ],
...
void main() {
  group('Part 1: Multi-Day Day-Sequence Integrity', () {
    ...
    testWidgets(
      'Empty day renders with friendly notice and does not disappear',
      (tester) async {
  ...
  group('Part 2: Manual Place Search & Add', () {
    testWidgets(
      'tap search reveals debounced input, searches destination context, and adds place',
      (tester) async {
        tester.view.physicalSize = const Size(430, 2000);
        tester.view.devicePixelRatio = 1;
        addTearDown(tester.view.resetPhysicalSize);
        ...
        final searchButton = find.byKey(
          const ValueKey('open-place-search-button'),
        );
        ...
        final searchInput = find.byKey(
          const ValueKey('manual-place-search-field'),
        );
        ...
        final addButton = find.byKey(
          const ValueKey('add-search-result-geo-hill-palace'),
        );
        ...
        expect(
          savedService.addedPlaceIds,
          contains('resolved-geo-hill-palace'),
        );
    ...
    testWidgets('Search error displays non-destructive message on failure', (
      tester,
    ) async {
      tester.view.physicalSize = const Size(430, 2000);
      tester.view.devicePixelRatio = 1;
      ...
      await tester.enterText(
        find.byKey(const ValueKey('manual-place-search-field')),
        'Museum',
      );
  ...
  group('Part 3: Interactive Map Performance & Progressive Rendering', () {
    testWidgets(
      'TripMapScreen renders base map and markers immediately with pre-passed state',
      (tester) async {
