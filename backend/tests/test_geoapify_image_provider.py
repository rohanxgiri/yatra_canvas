    ...
    return PlaceImageContext(
        ...
        sources=(
            PlaceImageSourceContext(
                source="geoapify",
                external_place_id="geoapify-abc",
...
async def test_reuses_imported_geoapify_image_without_details_call() -> None:
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        result = await GeoapifyImageProvider(
            "key", base_url="https://api.geoapify.com", client=client
        ).resolve(context({"geoapify_image": "https://images.example/hawa.jpg"}))
                ...
                "features": [
                    {
                        "properties": {
                            "wiki_and_media": {
                                "image": "https://images.example/details.jpg"
                            }
                        }
                    }
                ]
