...
from app.services.place_image_provider import PlaceImageCandidate
...
def seed(session: Session, count: int = 1):
    city = City(name="Jaipur", state="Rajasthan", country="India", latitude=26.91, longitude=75.79)
    session.add(city)
    places = [Place(city_id=city.id, name=f"Place {i}", category="heritage", latitude=26.91 + i / 10000, longitude=75.79) for i in range(count)]
...
async def test_positive_and_negative_cache_entries_are_reused() -> None:
    ...
    SQLModel.metadata.create_all(engine)
    candidate = PlaceImageCandidate(url="https://img/place.jpg", thumbnail_url=None, provider="wikimedia", provider_place_id="File:Place.jpg", source_url="https://commons.example", attribution="Credit", author="Author", license="CC BY", license_url="https://license.example")
    with Session(engine) as session:
...
async def test_expired_entry_refreshes_and_jaipur_20_warm_cache_is_faster() -> None:
    engine = create_engine("sqlite://", connect_args={"check_same_thread": False}, poolclass=StaticPool)
    SQLModel.metadata.create_all(engine)
    candidate = PlaceImageCandidate(url="https://img/place.jpg", thumbnail_url=None, provider="geoapify", provider_place_id=None, source_url=None, attribution=None, author=None, license=None, license_url=None)
    with Session(engine) as session:
        ...
        warm_result = await resolver.resolve_many(session, [place.id for place in places])
