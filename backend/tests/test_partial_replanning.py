...
def test_newly_generated_stop_defaults_to_planned(test_env):
    ...
    db_stop = session.exec(
        select(TripItinerary).where(TripItinerary.trip_id == trip.id)
    ).first()
...
def test_planned_to_completed_persists(test_env):
    ...
    # PATCH by stop_id
    resp = client.patch(
        f"/trips/{trip.id}/itinerary/stops/{stop.id}", json={"status": "COMPLETED"}
    )
    assert resp.status_code == 200
...
def test_planned_to_missed_and_skipped_persists(test_env):
    ...
    resp1 = client.patch(
        f"/trips/{trip.id}/itinerary/places/{p1.id}", json={"status": "MISSED"}
    )
    ...
    resp2 = client.patch(
        f"/trips/{trip.id}/itinerary/stops/{s2.id}", json={"status": "SKIPPED"}
    )
...
def test_terminal_statuses_cannot_be_reopened_and_missed_can_be_skipped(test_env):
    session, client = test_env
    trip, city, _ = _create_base_trip(session)
    places = [
        Place(
            id=uuid4(),
            city_id=city.id,
            name=f"Status Place {index}",
            category="heritage",
            latitude=24.57 + index * 0.01,
            longitude=73.68,
        )
    ...
    stops = [
        ...
        for index, (place, status) in enumerate(
            zip(places, ["COMPLETED", "SKIPPED", "MISSED"]), start=1
        )
    ]
    ...
    completed = client.patch(
        f"/trips/{trip.id}/itinerary/stops/{stops[0].id}",
        json={"status": "MISSED"},
    )
    skipped = client.patch(
        f"/trips/{trip.id}/itinerary/stops/{stops[1].id}",
        json={"status": "PLANNED"},
    )
    missed = client.patch(
        f"/trips/{trip.id}/itinerary/stops/{stops[2].id}",
        json={"status": "SKIPPED"},
    )
    ...
    assert missed.json()["status"] == "SKIPPED"
    session.expire_all()
    assert session.get(TripItinerary, stops[0].id).status == "COMPLETED"
    assert session.get(TripItinerary, stops[1].id).status == "SKIPPED"
...
def test_missed_alone_does_not_auto_move_place(test_env):
    ...
    resp = client.patch(
        f"/trips/{trip.id}/itinerary/stops/{stop.id}", json={"status": "MISSED"}
    )
    assert resp.status_code == 200
...
def test_move_missed_place_day1_to_day2_with_unrelated_day3_untouched(test_env):
    ...
    sB = TripItinerary(
        ...
        planned_departure_time=time(11, 15),
        status="COMPLETED",
    )
    sC = TripItinerary(
        id=uuid4(),
        ...
        day_number=1,
        visit_order=3,
        planned_arrival_time=time(11, 30),
        planned_departure_time=time(12, 30),
        status="MISSED",
    )
    sD = TripItinerary(
        id=uuid4(),
        ...
        planned_departure_time=time(14, 0),
        status="PLANNED",
    )
    ...
    day1_stops = session.exec(
        select(TripItinerary)
        .where(TripItinerary.trip_id == trip.id, TripItinerary.day_number == 1)
        .order_by(TripItinerary.visit_order)
    ).all()
    assert len(day1_stops) == 3
    assert (
        day1_stops[0].place_id == pA.id
        and day1_stops[0].status == "COMPLETED"
        and day1_stops[0].visit_order == 1
    )
    assert (
        day1_stops[1].place_id == pB.id
        and day1_stops[1].status == "COMPLETED"
        and day1_stops[1].visit_order == 2
    )
    # C is removed, D is re-ordered to visit_order 3
    assert (
        day1_stops[2].place_id == pD.id
        and day1_stops[2].visit_order == 3
        and day1_stops[2].status == "PLANNED"
    )

    # Verify Day 2:
    # Now has both E and C
    day2_stops = session.exec(
        select(TripItinerary)
        .where(TripItinerary.trip_id == trip.id, TripItinerary.day_number == 2)
        .order_by(TripItinerary.visit_order)
    ).all()
    ...
    day3_stops = session.exec(
        select(TripItinerary).where(
            TripItinerary.trip_id == trip.id, TripItinerary.day_number == 3
        )
    ).all()
...
def test_moved_place_respects_opening_hours_and_split_intervals(test_env):
    ...
    d2 = session.exec(
        select(TripItinerary).where(
            TripItinerary.trip_id == trip.id, TripItinerary.day_number == 2
        )
    ).first()
...
def test_infeasible_target_day_returns_structured_failure_and_leaves_data_untouched(
    test_env,
):
    session, client = test_env
    trip, city, days = _create_base_trip(session, num_days=2)
    ...
    stopExisting = TripItinerary(
        id=uuid4(),
        trip_id=trip.id,
        place_id=pExisting.id,
        day_number=2,
        visit_order=1,
        planned_arrival_time=time(9, 0),
        planned_departure_time=time(10, 0),
    ...
    d1 = session.exec(
        select(TripItinerary).where(
            TripItinerary.trip_id == trip.id, TripItinerary.day_number == 1
        )
    ).all()
    assert len(d1) == 1
...
def test_partial_replan_performance(test_env, place_count):
    ...
    assert t_elapsed < 4.0, (
        f"Partial replanning took {t_elapsed:.3f}s for {place_count}+1 places"
    )
